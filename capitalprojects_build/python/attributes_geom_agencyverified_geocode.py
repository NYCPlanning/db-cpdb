...
dcp_cpdb_agencyverified = pd.read_sql_query('SELECT address, borough, maprojid FROM dcp_cpdb_agencyverified WHERE geom IS NULL AND address IS NOT NULL AND borough IS NOT NULL;', engine)
...
def parse_output(geo):
    return dict(
        bin = geo.get('Building Identification Number (BIN) of Input Address or NAP', np.nan),
